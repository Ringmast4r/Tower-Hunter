#!/usr/bin/env python3
"""
TowerHunter v3.0 - Remote Mode
Runs locally but pulls cell/GPS data from ClockworkPi over SSH
"""
...
def ssh_cmd(cmd):
    """Run command on remote ClockworkPi via SSH"""
    full_cmd = ['sshpass', '-p', REMOTE_PASS, 'ssh', '-o', 'StrictHostKeyChecking=no',
                '-o', 'ConnectTimeout=5', f'{REMOTE_USER}@{REMOTE_HOST}', cmd]
    try:
...
class TowerHunter:
    ...
    def get_cell_data(self):
        data = {'mcc': '', 'mnc': '', 'lac': '', 'tac': '', 'cell_id': '',
                ...
                'rsrp': None, 'rsrq': None}
        try:
            output = ssh_cmd('sudo mmcli -m 0 --location-get')
            for line in output.split('\n'):
            ...
            output = ssh_cmd('sudo mmcli -m 0 --signal-get')
            for line in output.split('\n'):
                if 'rsrp:' in line:
    ...
    def run(self):
        ...
        print(f"\n{'='*60}")
        print(f"  TOWERHUNTER v3.0 - Remote Mode")
        print(f"  Connecting to ClockworkPi @ {REMOTE_HOST}")
        print(f"{'='*60}")
        print(f"  Fetching device info...")
        dev = self.get_device_info()
        with data_lock:
            current_data['device_info'] = dev
        print(f"  {dev['manufacturer']} {dev['model']}")
        print(f"  IMEI: {dev['imei']} | Phone: {dev['phone']}")
